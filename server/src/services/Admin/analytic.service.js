...
export const getUserStats = async () => {
  const { rows } = await pool.query(
    `SELECT
       ...
       AVG(bmi)::numeric(5,2) AS avg_bmi
     FROM profiles
...
export const getWorkoutStats = async () => {
  const { rows } = await pool.query(
    `SELECT
       ...
       AVG(kcal_burned)::numeric(7,1) AS avg_kcal
     FROM workout_logs
...
export const getAtRiskUsers = async () => {
  const { rows } = await pool.query(
    `SELECT u.id, u.name, u.email, p.bmi, p.age, p.medical_conditions
     FROM profiles p
     JOIN users u ON u.id = p.user_id
     WHERE p.bmi > 30 OR p.bmi < 18.5
       OR p.medical_conditions->>'high_blood_pressure' = 'true'

...
class ProgressModel {
  static async logProgress(userId, data) {
    ...
    const { rows } = await pool.query(
      `INSERT INTO progress_logs (
        user_id, log_date, weight_kg, body_fat_percentage,
        measurements, progress_photos, energy_level,
        sleep_hours, water_intake_liters, notes
      )
      ...
      DO UPDATE SET
        ...
        notes = COALESCE($10, progress_logs.notes),
        updated_at = NOW()
      RETURNING *`,
      [
        userId,
  ...
  static async getProgressTrends(userId, days = 30) {
    ...
    const trends = {
      ...
      avg_energy: rows.reduce((sum, r) => sum + (r.energy_level || 0), 0) / rows.length,
      avg_sleep: rows.reduce((sum, r) => sum + (r.sleep_hours || 0), 0) / rows.length,
      avg_water: rows.reduce((sum, r) => sum + (r.water_intake_liters || 0), 0) / rows.length,
    };
  ...
  static async countProgressLogs() {
    const { rows } = await pool.query(
      `SELECT COUNT(*) AS total FROM progress_logs`
    );
  ...
  static async avgWeight() {
    const { rows } = await pool.query(
      `SELECT AVG(weight_kg) AS avg_weight FROM progress_logs`
    );
  ...
  static async avgBodyFat() {
    const { rows } = await pool.query(
      `SELECT AVG(body_fat_percentage) AS avg_body_fat FROM progress_logs`
    );

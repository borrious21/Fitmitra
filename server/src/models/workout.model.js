...
// Fixes:
//   1. SQL injection: all INTERVAL usages now use parameterized queries ($N || ' days')::interval
//   2. getWorkoutHistory: now supports completedOnly filter
...
class WorkoutModel {
  static async logWorkout(userId, data) {
    ...
    const { rows } = await pool.query(
      ...
      RETURNING *`,
      [userId, workout_date, exercise_name, sets_completed, reps_completed,
  ...
  static async getWorkoutHistory(userId, filters = {}) {
    ...
    // FIX: completedOnly filter
    if (completed_only) {
      query += ` AND sets_completed IS NOT NULL AND sets_completed > 0`;
    }
  ...
  // FIX: Parameterized INTERVAL to prevent SQL injection
  ...
  static async checkStreakAchievements(userId, streak) {
    ...
    for (const milestone of milestones) {
      if (streak === milestone.days) {
        await this.awardAchievement(userId, milestone.type, milestone.name, { streak_days: streak });
      }
    }
  }
